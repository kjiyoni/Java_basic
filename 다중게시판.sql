...
CREATE TABLE board(
    ...
    description  VARCHAR2(400)                            -- 게시판 상세설명 (누가 만들었는지 등)
);
...
CREATE TABLE article (
    ...
    board_id      NUMBER(20)        NOT NULL,                             -- 게시글 소속 게시판번호 /  어느 게시판인지
    ...
    passwd        VARCHAR2(8)      NOT NULL,                             -- 게시글 비밀번호 //삭제 시 필요함
    group_no      NUMBER(7)        NOT NULL,                             -- 계층형 게시판 구조를 위한 게시글 그룹번호 / 신규글 작성시 게시글번호와 그룹번호 같게/ 게시글에 대한 답변까지 하나의 그룹으로
    level_no      NUMBER(2)        NOT NULL,                               -- 계층형 게시판 구조를 위한 그룹내 게시글 레벨 /신규글과 답변글을 판단하는 컬럼(신규글은0, 이후로는 1,2,3 등)화면상 들여쓰기
    order_no      NUMBER(3)        NOT NULL                              -- 계층형 게시판 구조를 위한 그룹내 게시글 순서/ 답변글 간에도 순서를 이용해서 상위 하위 조절 위함
--group~order까지 쿼리 만들어줘야 함
);
...
VALUES      (article_id_seq.NEXTVAL,
             ...
             (SELECT MAX(order_no) + 1
              ...
              WHERE  board_id = 10
                     AND group_no = 1));
--order번호는 서브쿼리 써야함(몇개의 글이 존재할지 모르니까 동적으로 생성 위해서 Max+1)
...
-- 등록전에 부모글보다 order_no이 큰  order_no을 1씩 증가시킨다   / 대댓글 달았을대 기존 글보다 위로 올리려고 할때 필요. (등록한 글이 3, 기존 글을 4로 만들기윔함)
UPDATE article 
...
WHERE  board_id = 10 
       ...
       AND order_no >      (SELECT order_no 
              ...
              SELECT article_id, subject,passwd,group_no,level_no, order_no FROM article WHERE group_no=1 ORDER BY order_no asc;
                       select* from article;
                       UPdate article 
                       set order_no = 5
                       where article_id=71;
                    DELETE article WHERE article_id = 77;
                    commit;
-- 대댓글 등록           
INSERT INTO article
            (article_id,
...
-- 페이징 처리를 위한 검색 게시글 검색  (mybatis에서 동적 sql 로 변경)
SELECT subject, 
       writer, 
       regdate, 
       hitcount 
